...
**当前阶段：M0–M6 全部完成（待你的 Sonatype 凭证后即可发布到 Maven Central）。** 安全审查无 HIGH/MEDIUM 发现；Maven 发布全套就绪（groupId `io.github.vincent-ye`、release profile 含 GPG 签名 + Central Publishing 插件、release workflow 一键发布）。流程见 [发布指南](docs/RELEASING.zh.md)。
...
- [快速开始](docs/GETTING_STARTED.zh.md) — SDK 与 Proxy 网关的上手示例
- [迁移指南](docs/MIGRATION.zh.md) — 从 LiteLLM (Python) 迁移 + 已知差异
- [能力矩阵](docs/CAPABILITIES.zh.md) — 六家供应商的能力与配置要点
- [架构设计](docs/DESIGN.zh.md) — 范围界定、技术选型、模块划分、核心抽象（SDK / Router / Proxy）、测试策略与风险
- [开发路线图](docs/ROADMAP.zh.md) — M0–M6 共约 21 周到 v1.0，每个里程碑含交付物与验收标准
- [发布指南](docs/RELEASING.zh.md) — Maven Central 发布的一次性准备 + 每次流程
